...
public class UserLoginServiceImpl implements UserLoginService {
  ...
  public UserLoginRespDTO login(UserLoginReqDTO requestParam) {
    ...
    String username;
    if (mailFlag) {
      LambdaQueryWrapper<UserMailDO> queryWrapper =
          Wrappers.lambdaQuery(UserMailDO.class).eq(UserMailDO::getMail, usernameOrMailOrPhone);
      username =
          Optional.ofNullable(userMailMapper.selectOne(queryWrapper))
              .map(UserMailDO::getUsername)
              .orElseThrow(() -> new ClientException("用户名/手机号/邮箱不存在"));
    } else {
      LambdaQueryWrapper<UserPhoneDO> queryWrapper =
          Wrappers.lambdaQuery(UserPhoneDO.class).eq(UserPhoneDO::getPhone, usernameOrMailOrPhone);
      username =
          Optional.ofNullable(userPhoneMapper.selectOne(queryWrapper))
  ...
  public Boolean hasUsername(String username) {
    ...
    if (hasUsername) {
      ...
      return instance
          .opsForSet()
          .isMember(USER_REGISTER_REUSE_SHARDING + hashShardingIdx(username), username);
    }
    return true;
  }
  ...
  public UserRegisterRespDTO register(UserRegisterReqDTO requestParam) {
    ...
    try {
      ...
      instance
          .opsForSet()
          .remove(USER_REGISTER_REUSE_SHARDING + hashShardingIdx(username), username);
  ...
  public void deletion(UserDeletionReqDTO requestParam) {
    ...
    try {
      ...
      UserDeletionDO userDeletionDO =
          UserDeletionDO.builder()
