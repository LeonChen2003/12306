...
public class PayTableComplexAlgorithm implements ComplexKeysShardingAlgorithm {
  ...
  public Collection<String> doSharding(
    ...
    if (CollUtil.isNotEmpty(columnNameAndShardingValuesMap)) {
      ...
      Collection<Comparable<?>> customerUserIdCollection =
      ...
      if (CollUtil.isNotEmpty(customerUserIdCollection)) {
        ...
        if (comparable instanceof String) {
          ...
          String dbSuffix =
              String.valueOf(hashShardingValue((Long) comparable % 1000000) % shardingCount);
  ...
  private int getShardingCount(final Properties props) {
    Preconditions.checkArgument(
        props.containsKey(SHARDING_COUNT_KEY), "Sharding count cannot be null.");
