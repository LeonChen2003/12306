...
public class OrderItemServiceImpl extends ServiceImpl<OrderItemMapper, OrderItemDO>
        ...
        if (CollectionUtil.isNotEmpty(orderItemDOList)) {
          orderItemDOList.forEach(
              o -> {
                ...
                int orderItemUpdateResult =
                    orderItemMapper.update(orderItemDO, orderItemUpdateWrapper);
                if (orderItemUpdateResult <= 0) {
                  throw new ServiceException(
                      OrderCanalErrorCodeEnum.ORDER_ITEM_STATUS_REVERSAL_ERROR);
                }
              });
        }
      }
  ...
  @Override
  public List<TicketOrderPassengerDetailRespDTO> queryTicketItemOrderById(
      TicketOrderItemQueryReqDTO requestParam) {
    LambdaQueryWrapper<OrderItemDO> queryWrapper =
