...
public @interface Idempotent {
  ...
  /**
   * 验证幂等类型，支持多种幂等方式 RestAPI 建议使用 {@link IdempotentTypeEnum#TOKEN} 或 {@link
   * IdempotentTypeEnum#PARAM} 其它类型幂等验证，使用 {@link IdempotentTypeEnum#SPEL}
   */
  ...
  /**
   * 设置防重令牌 Key 前缀，MQ 幂等去重可选设置 {@link IdempotentSceneEnum#MQ} and {@link IdempotentTypeEnum#SPEL}
   * 时生效
   */
  String uniqueKeyPrefix() default "";

  /**
   * 设置防重令牌 Key 过期时间，单位秒，默认 1 小时，MQ 幂等去重可选设置 {@link IdempotentSceneEnum#MQ} and {@link
   * IdempotentTypeEnum#SPEL} 时生效
   */
