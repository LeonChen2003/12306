...
/** 分布式缓存之操作 Redis 模版代理 底层通过 {@link RedissonClient}、{@link StringRedisTemplate} 完成外观接口行为 */
...
public class StringRedisTemplateProxy implements DistributedCache {
  ...
  public Boolean putIfAllAbsent(@NotNull Collection<String> keys) {
    DefaultRedisScript<Boolean> actual =
        Singleton.get(
            LUA_PUT_IF_ALL_ABSENT_SCRIPT_PATH,
            () -> {
    ...
    Boolean result =
        stringRedisTemplate.execute(
            actual, Lists.newArrayList(keys), redisProperties.getValueTimeout().toString());
  ...
  @Override
  public <T> T safeGet(
      @NotBlank String key, Class<T> clazz, CacheLoader<T> cacheLoader, long timeout) {
  ...
  @Override
  public void safePut(
      String key, Object value, long timeout, TimeUnit timeUnit, RBloomFilter<String> bloomFilter) {
